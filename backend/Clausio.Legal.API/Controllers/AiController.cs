...
public class AiController(IAiService aiService, IJudgmentAnalysisService judgmentAnalysis, IEmailService emailService) : ControllerBase
{
    ...
    private async Task StreamResponse(IAsyncEnumerable<string> stream, CancellationToken ct)
    {
        Response.ContentType = "text/event-stream";
        Response.Headers.Append("Cache-Control", "no-cache");
        Response.Headers.Append("X-Accel-Buffering", "no");
        try
        {
            // Collect full response for JSON endpoints to avoid split-token issues
            ...
            var sanitized = full.Replace("\n", "\\n").Replace("\r", "");
            await Response.WriteAsync($"data: {sanitized}\n\n", ct);
    ...
    [HttpPost("summary/stream/{caseId:guid}")]
    public async Task StreamSummary(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamSummaryAsync(caseId, ct), ct);

    [HttpPost("chronology/stream/{caseId:guid}")]
    public async Task StreamChronology(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamChronologyAsync(caseId, ct), ct);

    [HttpPost("contradictions/stream/{caseId:guid}")]
    public async Task StreamContradictions(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamContradictionsAsync(caseId, ct), ct);

    [HttpPost("research/stream/{caseId:guid}")]
    public async Task StreamResearch(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamResearchAsync(caseId, ct), ct);

    [HttpPost("actionplan/stream/{caseId:guid}")]
    public async Task StreamActionPlan(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamActionPlanAsync(caseId, ct), ct);

    [HttpPost("financial/stream/{caseId:guid}")]
    public async Task StreamFinancial(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamFinancialAsync(caseId, ct), ct);

    [HttpPost("readiness/stream/{caseId:guid}")]
    ...
    [HttpPost("recommendations/stream/{caseId:guid}")]
    public async Task StreamRecommendations(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamRecommendationsAsync(caseId, ct), ct);

    [HttpPost("witness/stream/{caseId:guid}")]
    public async Task StreamWitness(Guid caseId, CancellationToken ct)
        => await StreamResponse(aiService.StreamWitnessAsync(caseId, ct), ct);
}
