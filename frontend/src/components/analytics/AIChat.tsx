...
const QUICK_PROMPTS = [
  { icon: 'ti-file-text',      label: 'Case Summary',    message: 'Give me a complete case summary with parties, current stage, key strengths, weaknesses and recommended strategy.' },
  { icon: 'ti-scale',          label: 'Legal Research',  message: 'Find the 5 most relevant binding Supreme Court and High Court judgments for this case. For each judgment: citation, ratio decidendi, and exact words to use in court.' },
  { icon: 'ti-gavel',          label: 'Cross Exam',      message: 'Generate 15 targeted cross-examination questions for the opposing party main witness. Group by: income/assets, credibility, contradictions, and key facts.' },
  { icon: 'ti-target-arrow',   label: 'Next Steps',      message: 'What are the 7 most critical actions I must take in the next 14 days for this case? For each: action, legal basis, deadline, and who is responsible.' },
  { icon: 'ti-shield-check',   label: 'Evidence Gaps',   message: 'What documents and evidence are missing from this case? For each gap: what document, why it matters legally, and how to obtain it.' },
  { icon: 'ti-currency-rupee', label: 'Maintenance',     message: 'Calculate recommended maintenance using Rajnesh v. Neha (2020) standard. Show: minimum, recommended, maximum, and pendente lite amount with calculation basis.' },
]
...
export default function AIChat() {
  ...
  function clearChat() { setMessages([]); setInput(''); setError('') }
  ...
  const handleVoiceInput = async () => {
    ...
    try {
        ...
        if (e.data.size > 0) {
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (data.text?.trim()) setInput(originalText + (originalText ? ' ' : '') + data.text.trim())
            }
          } catch {}
        }
      }
      ...
      recorder.start(1500)
      setIsListening(true)
    } catch { alert('Microphone access denied.'); setIsListening(false) }
  }
  ...
  return (
    <div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ padding: 20, minHeight: 380, maxHeight: 560, overflowY: 'auto' }}>
          ...
          {messages.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 340, gap: 16 }}>
              ...
              <h3 style={{ fontSize: 20, fontWeight: 600, color: '#0f172a', margin: 0 }}>Ask Clausio anything</h3>
              <p style={{ color: '#64748b', fontSize: 13, textAlign: 'center', maxWidth: 400, lineHeight: 1.6 }}>
                {selectedCaseId
                  ? 'Case loaded. Ask me anything or use the quick prompts above.'
            ...
            <div key={i} style={{ marginBottom: 20 }}>
              {m.role === 'user' ? (
                <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '0 4px' }}>
                  <div style={{ maxWidth: '85%', background: '#ffffff', color: '#0f172a', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 16px rgba(0,0,0,0.04)', borderRadius: 18, borderBottomRightRadius: 4, padding: '12px 16px', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '0 4px', maxWidth: '94%' }}>
                  ...
                  <div style={{ flex: 1, position: 'relative', paddingRight: 32 }}>
                    {/* ── FLASH CARD OUTPUT ── */}
                    <AIResponseFormatter content={m.text} />
                    <button onClick={() => copyMessage(i, m.text)}
                      style={{ position: 'absolute', top: -4, right: 0, background: 'none', border: 'none', cursor: 'pointer', color: copied === i ? '#22c55e' : '#94a3b8', fontSize: 14, padding: 4 }}>
                      <i className={`ti ${copied === i ? 'ti-check' : 'ti-copy'}`} />
                    </button>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '0 4px', maxWidth: '90%' }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.6, margin: 0, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <i className="ti ti-loader animate-spin" style={{ fontSize: 16 }} /> Thinking...
                </p>
              </div>
            </div>
          )}
        ...
        {/* Input */}
        <div style={{ padding: '12px 14px', borderTop: '1px solid #f1f5f9', position: 'relative' }}>
          <div className="apple-intelligence-chat-pill">
            <textarea rows={1} className="apple-intelligence-input-text" value={input}
            ...
            <button onClick={handleVoiceInput}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: isListening ? '#ef4444' : '#64748b', marginRight: 8, transition: 'color 0.2s' }}>
              <i className="ti ti-microphone" style={{ fontSize: 18 }} />
            </button>
            <button onClick={() => send(input)} disabled={loading || !input.trim()} className="apple-intelligence-send-btn"
