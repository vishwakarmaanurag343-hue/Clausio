...
export default function PaymentTracker({ cases, onRefresh }: Props) {
  ...
  async function recordPayment() {
    ...
    try {
      ...
      setShowForm(false); setInvoiceId(''); setAmount(0); setReference(''); setNotes('')
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 10, padding: '10px 18px', fontSize: 13 }}>
          Total Received: <strong style={{ color: '#15803d', fontSize: 20, marginLeft: 6 }}>{fmt(totalReceived)}</strong>
        </div>
      ...
      {showForm && (
        <div style={{ background: '#fff', border: '1.5px solid #86efac', borderRadius: 16, padding: 24, marginBottom: 20 }}>
          <div style={{ fontWeight: 700, color: '#0f172a', fontSize: 16, marginBottom: 18 }}>Record Payment</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div><label style={lbl}>Invoice *</label>
              <select value={invoiceId} onChange={e => onInvoiceChange(e.target.value)} style={inp}>
                ...
                {invoices.filter(i => i.status !== 'Paid' && i.status !== 'Cancelled').map(i => (
                  <option key={i.id} value={i.id}>{i.invoiceNumber} — {i.caseName} ({fmt(i.amountPending ?? i.totalAmount)} pending)</option>
                ))}
              </select>
            </div>
            ...
            <div><label style={lbl}>Payment Mode</label>
              <select value={mode} onChange={e => setMode(e.target.value)} style={inp}>
                {['Cash','UPI','Bank Transfer','Cheque','NEFT','RTGS'].map(m => <option key={m}>{m}</option>)}
              </select>
            </div>
            <div><label style={lbl}>Reference / UTR</label>
              <input value={reference} onChange={e => setReference(e.target.value)} placeholder="Optional" style={inp} />
            </div>
            <div><label style={lbl}>Paid On</label>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
            ...
            <button onClick={recordPayment} disabled={saving} style={{ ...priBtn, opacity: saving ? 0.7 : 1 }}>
      ...
      {loading
        ? <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8', fontSize: 13 }}>Loading payments...</div>
        : payments.length === 0
        ? (
        ...
        : (
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr 60px', padding: '12px 16px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.5 }}>
              <div>Invoice</div><div>Amount</div><div>Mode</div><div>Reference</div><div>Date</div><div></div>
            </div>
            {payments.map((p, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr 60px', padding: '14px 16px', borderBottom: i < payments.length-1 ? '1px solid #f1f5f9' : 'none', alignItems: 'center', fontSize: 13 }}>
                <div>
                  <div style={{ fontWeight: 700, color: '#2563eb' }}>{p.invoiceNumber ?? '—'}</div>
                  <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{p.caseName ?? '—'}</div>
                </div>
                <div style={{ fontWeight: 800, color: '#16a34a', fontSize: 16 }}>{fmt(p.amount)}</div>
                <div>
                  <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: '#f0fdf4', color: modeColors[p.mode] ?? '#64748b' }}>
                    {p.mode ?? '—'}
                  </span>
                </div>
                <div style={{ fontSize: 12, color: '#64748b' }}>{p.reference ?? '—'}</div>
                <div style={{ color: '#64748b', fontSize: 12 }}>{p.paidOn ? new Date(p.paidOn).toLocaleDateString('en-IN') : '—'}</div>
                <div>
...
const priBtn: React.CSSProperties = { flex: 2, padding: '11px', borderRadius: 10, border: 'none', background: '#16a34a', color: '#fff', cursor: 'pointer', fontWeight: 700, fontFamily: 'inherit', fontSize: 13 }
